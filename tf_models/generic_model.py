...
class GenericModel(Model):
    def __init__(self, hyper_params_filepath, create_model_fn, create_loss_fn):
        ...
        :param create_loss_fn: A function that gets in the output tensors of the create model the labels and the
            ...
            (train_op, loss_op, validation_loss_op = create_loss_fn(model_train, model_labels, labels, validation_labels, hyperparams))
        """
    ...
    def _create_model(self, input_tensor, reuse_weights):
        return self.__create_model(input_tensor, reuse_weights, self.hyper_params)

    def _create_loss(self, labels, validation_labels=None):
        train_op, loss_op, validation_loss_op = self.__create_loss(self.model_train, self.model_validation, labels, validation_labels, self.hyper_params)
